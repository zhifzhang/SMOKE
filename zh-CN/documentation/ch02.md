...
- **固定区域源/非点源：** 被视为在空间范围内散布（通常是县或空气区域）且不可移动（与移动源相比）的源。由于不可能收集每个排放源数据，因此在较大区域范围内进行估算。EPA引入术语“非点源”来代替“固定区域源”，以避免与术语“面源”混淆（术语“面源”在有毒污染物管理领域被用作监管术语）。但是，“非点源”这一术语尚未被基本污染物清单/模型社区所接受（到目前为止）。因此，在本手册中，当它们位于基本污染物清单中时，我们将使用术语“固定区域源”来指代这些来源，而当它们处于有毒污染物清单中时，将使用术语“非点源”来指代这些来源。非点源或固定区域源的例子是住宅供暖和建筑涂料。而诸如干洗设施之类的多种污染源则可以被视为固定区域源/非点源，也可以被视为点源。尤其是，有毒污染物清单包含了许多小的排放源（基于排放量），这些小的排放源由于已知并提供了位置而未被列入非点源清单。
...
SMOKE中的排放处理分为四个处理类别：区域源、生物源、移动源和点源。SMOKE使用的这些类别的定义与排放清单的定义不同。下表中列出了清单污染源类别、SMOKE可以处理的清单类型（基本污染物、颗粒物、有毒污染物和活动数据）、SMOKE可以接受的时间分辨率，以及应用于处理清单的SMOKE处理类别。

**表2.1 清单污染源类别以及SMOKE处理能力和类别**
...
每个清单来源类别都有来源特征，来源属性，数据值和数据属性。 [*来源特征*] {。emphasis}对于每种库存来源类别都是唯一的，并且还会将库存中的一种来源与另一种来源区分开。 [*来源属性*] {。emphasis}进一步描述了具有其他可用于排放处理的信息的来源，例如点源烟道气出口高度和温度。 [*数据值*] {。emphasis}是排放值或活动值。 [* data attribute *] {。emphasis}是有关数据值的附加信息，例如已应用于源的控件的排放减少百分比。在以下小节中，我们总结了SMOKE用于每个清单类别的源特征和属性以及数据值和属性。

<a id=2.3.2.2.1> </a>

##### 2.3.2.2.1非点/固定区域和非道路移动设备（SMOKE类别：区域）

-[**来源特征：**]对于所有典型清单，标识这些来源的来源特征为国家/州/县代码，SCC和/或GEOCODE \ _LEVEL \ [1-4 \]。请参见[第2.3.4节“国家，州和县代码”]（ch02s03s04.html“ 2.3.4。国家，州和县代码”）和[第2.3.5节“源分类代码”]（ch02s03s05 .html“ 2.3.5。源分类代码”）和[第2.3.7节，“地理代码（GEOCODE \ _LEVEL \ [1-4 \]”]]（ch02s03s07.html“ 2.3.7。地理代码（GEOCODE_LEVEL [1 -4]“）获取更多信息。

-[**可选的源特征：**] SMOKE还可以使用与SMOKE区域源相同建模域中的预栅格数据；这将在[第2.8.2.2节“预排放”]中详细描述（ch02s08s02.html＃sect_concepts_pregridded_data“ 2.8.2.2。预排放”）。在这种情况下，源特性和属性（国家/州/县代码和SCC）是SMOKE中使用的[* not *] {。emphasis}。 SMOKE还可以使用来自不同建模域的预栅格化数据以及地理代码（GEOCODE \ _LEVEL \ [1-4 \]）和ARINV文件中的源信息来为每个网格单元指定源特性和关联的源属性。

-[**来源属性：**]库存年度与库存输入文件中的所有来源相关联。此外，SMOKE还分配了一个时区（请参阅[第2.9.14节，“分配国家代码，年和时区”] [ch02s09s14.html“ 2.9.14。分配国家代码，年和时区”））和一种用于时间轮廓标准化的方法（请参阅[第2.9.13节，“设置工作日平均方法”]（ch02s09s13.html“ 2.9.13。设置工作日平均方法”）。仅在非点毒物清单中，标准行业分类（SIC）代码，最大可实现控制技术（MACT）代码和北美工业分类系统（NAICS）代码是可选的来源属性。 SMOKE会读取NAICS代码，但此时不使用它。此外，非点清单中有一个“ [来源类型]”字段，用于标识主要和清洁空气法（CAA）部分112的区域来源。有关SIC代码的说明，请参见[第2.3.6节，“标准工业分类代码”]（ch02s03s06.html“ 2.3.6。标准工业分类代码”），[第2.3.8节，“可实现的最大控制技术代码”]。 （ch02s03s08.html“ 2.3.8。最大可达到的控制技术代码”），以获得有关MACT代码的更多信息，以及[第2.3.9节，“源类型：主要和112部分区域的源”]]（ch02s03s09.html“ 2.3.9源类型：主要和第112节区域源”），以获取有关源类型的更多信息。我们将CAA部分112的区域源简称为“ [section-112区域的源]”。

-[**数据：**] SMOKE可以读取非点/固定区域和非道路清单的标准，微粒和有毒污染物的排放数据。 SMOKE系统不受所读取污染物的限制（尽管在[2.3.1节，“清单数据类型”]中有典型示例（ch02s03.html＃sect_concepts_inv_data_types“ 2.3.1。库存数据类型”））。 SMOKE接受年度排放数据，平均日排放数据或两者（尽管并非所有输入格式都支持所有类型）。 SMOKE也可以读取排放因子值，但SMOKE对此不执行任何操作。

-[**数据属性：**]非点/固定区域和非道路移动源的清单可以包含每种污染物的控制效率，规则渗透和规则有效性信息。如果提供，SMOKE将使用这些数据；否则，它将设置默认值，该默认值指示未对库存污染物数据进行基于控制的调整。默认值以文件格式列出在[第8章，* SMOKE输入文件*]（ch08.html“第8章。SMOKE输入文件”）中。

<a id=2.3.2.2.2></a>

##### 2.3.2.2.2公路移动（SMOKE类别：移动）

-[**来源特征：**]对于公路上的移动清单，识别这些来源的最小来源特征是国家/州/县代码以及SCC [*或*] {。emphasis}道路类别和车辆类型代码。提供SCC后，必须遵循特定的模式才能包含道路类别和车辆类型代码（请参见[第2.8.4.1节，“道路移动MOVES SCC的特殊方法”]（ch02s08s04.html＃sect_concepts_onroad_sccs“ 2.8.4.1。公路移动MOVES SCC的特殊方法”）。当道路等级和车辆类型代码直接提供给SMOKE时，SMOKE会将其转换为SCC值。

-[**可选的源特性：**]链接代码也可以标识道路上的源。该代码在每个县和SCC（或道路类别/车辆类型组合）中必须唯一。

-[**来源属性：**]库存年度与库存输入文件中的所有来源相关联。此外，SMOKE还分配了一个时区（请参阅[第2.9.14节，“分配国家代码，年和时区”] [ch02s09s14.html“ 2.9.14。分配国家代码，年和时区”））和一种用于时间轮廓标准化的方法（请参阅[第2.9.13节，“设置工作日平均方法”]（ch02s09s13.html“ 2.9.13。设置工作日平均方法”）。对于具有链接代码的源，SMOKE将使用纬度-经度（纬度）值或通用横轴墨卡托（UTM）坐标系中的坐标来使用链接的开始和结束坐标。

-[**数据：**]可以读取道路移动清单中的标准，微粒和有毒污染物的排放数据。对于读取的污染物，SMOKE不受限制（尽管典型示例已在[第2.3.1节，“清单数据类型”]（ch02s03.html＃sect_concepts_inv_data_types“ 2.3.1。库存数据类型”）中给出）。 SMOKE接受年度排放数据，平均日排放数据或两者（尽管并非所有输入格式都支持所有类型）。

 此外，道路上的移动清单可能包含VMT和平均速度活动数据，当用户希望SMOKE运行MOVES来计算排放量时，就需要这些数据。预先计算的排放量和VMT数据的组合也可以输入SMOKE，但您有责任防止排放量重复。如果您输入了与使用SMOKE来计算飞行中的排放量相同的源的预先计算的排放量，方法是将MOVES的道路排放因子乘以每小时VMT，将MOVES的网络外排放因子乘以年度车辆人口。
...
-[**来源特征：**]点来源的来源特征取决于库存输入格式。 SMOKE每行一个记录（ORL）和平面文件10格式（FF10）格式按国家/州/县代码，工厂代码，点代码，堆栈代码，段代码和SCC标识源。 95。

-[**可选的源特性：**] SMOKE在一个工厂内最多可以支持五个位置标识符，尽管在当前实现的任何输入文件格式中使用最多的是四个。

-[**来源属性：**]与其他来源类别一样，清单年份与清单输入文件中的所有来源相关联。 SMOKE还分配了一个时区（请参阅[第2.9.14节，“分配国家代码，年和时区”]（ch02s09s14.html“ 2.9.14。分配国家代码，年和时区”））和一种方法（请参阅[第2.9.13节，“设置工作日平均方法”]（ch02s09s13.html“ 2.9.13。设置工作日平均方法”））。此外，点源具有以下必需的源属性，这些属性与其他源类别无关：纬度，经度，烟囱高度，烟囱直径（在出口位置），烟气出口速度和烟气出口温度。最后，SMOKE还使用以下可选源属性：SIC代码，MACT代码，工厂描述，排放释放类型点（例如，水平烟囱，逃犯），源类型（主要或112部分区域），法规信息办公室系统（ORIS）识别码和锅炉识别码。另外，仅ORL格式支持MACT代码和源类型。请参阅[第2.3.6节，“标准工业分类代码”]（ch02s03s06.html“ 2.3.6。标准工业分类代码”），[第2.3.8节，“可实现的最大控制技术代码”]（ch02s03s08.html“ 2.3 .8。最大可达到的控制技术代码”）和[第2.3.9节，“源类型：主要和112部分区域的源”]（ch02s03s09.html“ 2.3.9。源类型：主要和112部分区域源” “） 了解更多信息。

-[**数据：**]可以读取标准清单，微粒和有毒污染物的排放数据，以获取点清单。对于读取的污染物，SMOKE不受限制（尽管典型示例已在[第2.3.1节，“清单数据类型”]（ch02s03.html＃sect_concepts_inv_data_types“ 2.3.1。库存数据类型”）中给出）。 SMOKE接受年度排放数据，平均日排放数据或两者。可选地，可以使用特定日期或特定小时的记录来提供点源排放数据。这些数据的格式在[第8.2.6节，“ PTDAY”：点源日特定排放量]]中（ch08s02s06.html“ 8.2.6。PTDAY：点源日特定排放量”）和[第8.2节.7，“`PTHOUR`：特定点源小时的排放量”]（ch08s02s07.html“ 8.2.7。PTHOUR：特定点源小时的排放量”）。

-[**数据属性：**] EGU和非EGU点源可以包含每种污染物的控制效率和规则有效性信息。如果提供，SMOKE将使用这些数据；否则，它将设置默认值，该默认值指示未对库存污染物数据进行基于控制的调整。默认值以文件格式列出在[第8章，* SMOKE输入文件*]（ch08.html“第8章。SMOKE输入文件”）中。

<a id=2.3.2.2.4></a>

##### 2.3.2.2.4预先计算的羽状上升的野火（烟雾类别：点）

-[**来源特征：**]带有预先计算的羽状流的野火由国家/州/县代码和火名称标识。

-[**可选的源特性：**]野火源没有可选的源特性。

-[**来源属性：**]与其他来源类别一样，清单年份与清单输入文件中的所有来源相关联。 SMOKE还分配了一个时区（请参阅[第2.9.14节，“分配国家代码，年和时区”]（ch02s09s14.html“ 2.9.14。分配国家代码，年和时区”））和一种方法（请参阅[第2.9.13节，“设置工作日平均方法”]（ch02s09s13.html“ 2.9.13。设置工作日平均方法”））。另外，野火源需要纬度和经度源属性。最后，针对野火源[*必须*] {。emphasis}提供了额外的特定于小时的源属性，用于表层排放量，羽流底部的高度和羽流顶部的高度。这些小时特定的属性使用[第8.2.7节“`PTHOUR`：点源小时特定的排放量”]中所述的点源小时特定格式提供给SMOKE（ch08s02s07.html“ 8.2.7。PTHOUR：点源小时特定排放量”）。

-[**数据：**]野火源清单可能包含标准和颗粒污染物。对于读取的污染物，SMOKE不受限制（尽管典型示例已在[第2.3.1节，“清单数据类型”]（ch02s03.html＃sect_concepts_inv_data_types“ 2.3.1。库存数据类型”）中给出）。必须使用[第8.2.6节“`PTDAY`：点源日特定排放量”]中指定的点源格式，将这些数据作为特定日期或特定小时的排放值提供（ch08s02s06.html“ 8.2.6。PTDAY” ：点源日特定排放量”）和[第8.2.7节“ PTHOUR”：点源时特定排放量”]（ch08s02s07.html“ 8.2.7。PTHOUR：点源时特定排放量”）。

-[**数据属性：**]没有数据属性与野火源相关联。

<a id=2.3.2.2.5> </a>

##### 2.3.2.2.5具有内部羽状上升计算的野火（“烟雾”类别：点）

-[**来源特征：**]带有内部羽状上升计算的野火由国家/州/县代码，火灾识别，火灾名称，位置识别和SCC识别。

-[**可选的火源特性：**]火源有可选的火源特性，例如燃烧的材料，植被类型，燃烧的面积大小，燃料装载和火的开始/结束时间。燃烧面积和燃料装载量用于计算特定于火灾的烟羽上升。需要火灾起止时间来调整排放的每小时时间分布。

-[**来源属性：**]与其他来源类别一样，清单年份与清单输入文件中的所有来源相关联。 SMOKE还分配了一个时区（请参阅[第2.9.14节，“分配国家/地区代码，年和时区”]]（ch02s09s14.html“ 2.9.14。分配国家/地区代码，年和时区”））， -根据火灾的开始时间和结束时间对时间轮廓进行归一化。此外，野火源需要纬度和经度源属性来定位火源。请注意，将假定一场火灾的所有排放物均来自包含火灾纬度和经度的单个网格单元。最后，还提供了以上列出的针对火源[* must *] {。emphasis}的其他特定于一天的源属性，用于计算每种火的热通量，该热通量用于估算表层排放量，羽的底部和羽的顶部的高度。在[第4.4.17节，“火灾的羽状上升计算”]中查看有关如何处理的更多信息（ch04s04s17.html“ 4.4.17。火灾的羽状上升计算”）

-[**数据：**]火源清单可能包含标准和颗粒污染物。对于读取的污染物，SMOKE不受限制（尽管在[8.2.8.3节“ ORL FIRE格式”]中提供了典型示例（ch08s02s08.html＃sect_input_ptinv_fire“ 8.2.8.3。ORL FIRE格式”））。这些数据必须使用[第8.2.6.2节“ ORL FIREEMIS排放格式”]（ch08s02s06.html＃sect_input_ptday_fireemis“ 8.2.6.2。ORL FIREEMIS排放格式”）中指定的点源格式作为特定日期的排放值提供。

-[**数据属性：**]没有数据属性与野火源相关联。

<a id=2.3.2.2.6></a>

##### 2.3.2.2.6生物土地利用（烟的类别：生物基因）

-[**源特征：**]生物排放数据不能像先前描述的源类型那样完美地适合源特征范式。从土地利用数据开始估算生物源的排放量，该数据可用于BELD2和BELD3处理。 BELD2数据可以按美国州/县和BELD2土地使用类别获得，也可以按网格单元和BELD2土地使用类别获得。 BELD3土地利用数据可通过北美和中美洲的1公里网格单元以及BELD3土地利用类别获得。

-[**可选来源特征：**]生物土地利用数据不包括可选来源特征。数据可以按州/县分类，也可以按网格分类。

-[**来源属性：**]没有生物土地利用数据的来源属性。

-[**数据：**]生物土地利用数据包括与县或网格单元中每种土地利用类型相关的部分。
...
SMOKE支持用于标准，微粒，有毒物质和活动数据清单的多种清单格式，这些格式在[第8.2节，“清单文件”]（ch08s02.html“ 8.2。清单文件”）中进行了详细描述。在这里，我们对这些格式进行了简要介绍，当您在本章其余部分以及[第8章，* SMOKE输入文件*]（ch08.html“第8章。SMOKE输入文件”）。此处描述的所有格式均为文本文件。要将数据转换为这些格式，最好的方法是使用数据库或电子表格程序重新格式化并以请求的格式输出数据。 SMOKE附带没有标准的格式转换方法。

在以下段落中，我们描述了可用于非点/固定区域，非道路移动，道路移动，点和点野火源的格式。

-[**非点/固定区域源：**] SMOKE支持两种格式的非点/固定区域源。 ORL和FF10（平面文件10）格式是列表定向（用逗号或分号分隔），这些文件格式可用于表示许多不同的来源。文件的标题指示文件中包含哪些源数据。

-[**非道路移动资源：**]非道路移动资源有三种可用的广告资源格式。 FF10（平面文件10）格式是列表定向的（逗号或分号分隔），并且文件头用于指示非道路移动源数据在文件内。

-[**道路上的移动源：**]平面文件10（FF10）格式为列表定向（以逗号或分号分隔），并包含活动清单，例如VMT，速度和车辆人口数据。此格式需要VMT，SPEED和VPOP库存数据。

-[**点数来源：**] SMOKE具有年度或平均日清单，特定日期的清单和特定小时的清单的格式。对于年度或平均日清单，可以将ORL和FF10格式用于标准清单，微粒清单和有毒物质清单。最后，CEM数据格式可用于特定日期或特定小时的数据：SMOKE使用年度清单文件中的ORIS代码和锅炉代码来匹配CEM数据文件中的来源。

-[**野火源：**]您可以使用两种方法将野火数据提供为使用ORL和FF10点源格式的SMOKE点源。
     
 -[**预先计算的羽状上升方法：**]由于某些字段不适用于野火源，因此必须将其留空（例如堆栈参数）。当使用提供的野火数据作为点源时，您还必须使用FF10特定日期和特定时间的格式提供特定日期或特定时间的野火排放以及特定时间的预先计算的羽状上升。

 -[**内部羽状上升计算方法：**]需要两个单独的清单文件，这些文件以经过修改的ORL格式提供：（1）具有火灾特定特征的火灾清单，包括县/州/县，火灾识别，位置坐标，火灾名称，SCC等，如[第8.2.8.3节“ ORL FIRE格式”]（ch08s02s08.html＃sect_input_ptinv_fire“ 8.2.8.3。ORL FIRE格式”）所述，以及（2）特定日期的火灾数据，包括燃烧面积，燃料装载量和起火的小时数/小时数（[第8.2.6.2节“ ORL FIREEMIS排放格式”]（ch08s02s06.html＃sect_input_ptday_fireemis“ 8.2.6.2。ORL FIREEMIS排放格式”））。与上面列出的方法不同，此方法在内部使用燃烧区域的大小和燃料负载来估计羽流上升，并在火灾的开始和结束时间调整时间分布。请参阅[第4.4.17节，“火灾的羽状上升计算”]中的详细信息（ch04s04s17.html“ 4.4.17。火灾的羽状上升计算”）

<a id=2.3.4></a>

### 2.3.4国家，州和县代码

SMOKE使用6位整数代码来标识特定来源的国家/地区，州（或省）和县（或其他地区）。输入到SMOKE的大多数美国清单都有5位数字的美国联邦实施计划标准（FIPS）州和县代码。所有库存输入格式都经过修改，以包括一个特殊的抬头记录，您可以使用该记录指定国家/地区，从而有效地为库存提供了SMOKE使用的6位代码。 6位数系统设计用于美国各州和县以及加拿大和墨西哥，但也可以用于其他用途。 SMOKE用于代码的格式为：

！[]（images \ concepts \ costcy_html.jpg）

SMOKE安装程序设置为使用在`COSTCY'或`GEOCODE_LEVEL4`（如果是USE \ _EXP \ _GEOCODES Y）文件中定义的以美国为中心的代码，其中包含代码及其相关名称和时区设置。在此文件中，美国国家/地区代码为零，这使得美国国家/地区/州代码可以与美国清单中显示的FIPS州/州代码相同。有关以下内容的更多信息，请参见[第8.10.2节，“`COSTCY`：国家，州和县名称和数据”]（ch08s10s02.html“ 8.10.2。COSTCY：国家，州和县名称和数据”） `COSTCY`文件格式。

要更改SMOKE中国家，州或县代码的含义，必须修改`COSTCY`或`GEOCODE_LEVEL4`（如果为USE \ _EXP \ _GEOCODES Y）文件，以使用与每个国家，州，县相关的不同名称。或部落编号。所有SMOKE输入文件也必须使用此新的编号方案，包括清单文件和交叉引用文件。

SMOKE中国家/地区代码的可接受值为0到9。州代码的可接受值为1到99。县级代码的可接受值为1到999。不接受字母代码，因为SMOKE将这些值存储为整数。

<a id=2.3.5> </a>

### 2.3.5源分类代码

EPA使用源分类代码（SCC），区域和移动源（AMS）代码对不同类型的人为排放活动进行分类。 SCC的点源有8位数字，而AMS代码有10位数字，有时还包括前导“ [A]”作为第11个字符。在SMOKE中，我们将两种代码都称为“ [SCC]”，而忽略区域代码和移动代码中的前导“ [A]”。此外，SMOKE允许非点和点毒物清单在同一清单输入文件中使用8位和10位SCC，因为1999年NEI的非点和点清单中都包含8位和10位代码。 HAP。从SMOKE v4.0开始，SMOKE及其输入文件中SCC的最大字段宽度为20个字符。仍然支持8位或10位SCC，但如果SCC大于10位，则不支持SCC分层方法。

对于大小不超过10个字符的SCC，代码使用分层系统，随着您从左到右移动，代码的定义变得越来越具体。 （注意：如果SCC的长度大于10个字符，则不使用层次结构。）对于10个字符或更少的SCC，了解代码的层次结构很重要，因为您可以在构建交叉引用文件中利用层次结构将排放处理因子分配给清单排放源。在下图中，级别1是最不具体的，级别4是最具体的。

8位点源代码的代码结构为：

！[]（images \ concepts \ scc_point_html.jpg）

点源活动和相应的SCC的示例可以直接从SMOKE的SCC描述文件（`SCCDESC`）中获取：“ [外部燃烧锅炉；发电；褐煤；散布炉灶]”由10100306表示。该描述以及SCC级别：

！[]（images \ concepts \ scc_example_html.jpg）

同样，10位区号和移动源代码的代码结构为：

！[]（images \ concepts \ scc_nonpoint_html.jpg）

尽管在实践中，SMOKE将SCC视为字符串，但实际上库存和交叉引用文件中的值通常是数字。在关于交叉引用和配置文件的[第2.4节，“交叉引用和配置文件”]（ch02s04.html“ 2.4。交叉引用和配置文件”）中，我们解释了SMOKE如何使用这些层次结构以及如何在SMOKE中使用它们。准备SMOKE输入文件。

对于道路移动源，与其他源类别相比，对SCC的处理有些不同。我们将在[第2.8.4.1节“公路移动MOVES SCC的特殊方法”]（ch02s08s04.html＃sect_concepts_onroad_sccs“ 2.8.4.1。公路移动MOVES SCC的特殊方法”）中对此进行详细说明。

<a id=2.3.6></a>

### 2.3.6标准行业分类代码

尽管在EPA的建筑排放清单中SIC代码已被NAICS代码取代，但SIC代码仍用于排放处理中。从SMOKE v4.0开始，SIC代码的长度最多为20个字符，但是对于大于4位数字的SIC代码，则不使用分层系统。对于大小为4位数的SIC，SMOKE将2级层次结构系统识别为适用于生长，控制和化学形态因子的应用。这两个代码级别如下所示。

！[]（images \ concepts \ sic_html.jpg）

<a id=2.3.7> </a>

### 2.3.7地理代码（GEOCODE \ _LEVEL \ [1-4 \]

可以使用GEOCODE \ _LEVEL \ [1-4 \]文件将地理代码指定为用户所需的详细程度。 GEOCODE \ _LEVEL1包含清单（CCC）中每个国家/地区的三个字符代码。 GEOCODE \ _LEVEL2包含六个字符代码，用于用户希望在清单（CCCSSS）中跟踪的每个状态。 GEOCODE \ _LEVEL3包含用户要在清单中跟踪的每个县的九个字符代码（CCCSSSYYY）。 GEOCODE \ _LEVEL4包含用户想要在清单中跟踪的每个部落区域的十二个字符代码（CCCSSSYYYTTT）。
...
以下引文解释了MACT代码是从EPA摘录的，以解释什么是MACT代码以及为什么在某些清单中而不是在其他清单中使用它们：

为了通过最大可实现控制技术（MACT）标准评估EPA在减少空气有毒物质排放方面的进展，并确定可能被建模为残留风险评估一部分的污染源，在NTI中将受MACT标准约束的设施内的运营确定为4位数的MACT码。 [* \ [请注意，术语NTI（国家有毒物质​​清单）此后已被NEI取代，并且代码现在为6位数字\] *] {。强调

MACT类别是根据《清洁空气法》（有害空气污染物的国家排放标准）第112（d）条已经或正在制定排放限制的类别。 EPA通过其MACT计划设定了源类别，基于技术的标准，从而大大减少了HAP的排放。 EPA的ATW网站包含有关MACT来源类别和MACT计划的信息（www.epa.gov/ttn/atw/eparules.html）。使用MACT代码标记数据可以使EPA确定归因于MACT程序的减少量。 NTI将与MACT源类别相对应的MACT代码与固定专业和\ [section-112 \]区域源数据相关联。 MACT代码在点源数据中的过程级别或站点级别分配，例如，市政垃圾燃烧器（MWC）的MACT代码在站点级别分配，而石油精炼厂催化裂化的MACT代码在站点级别分配。流程级别。 MACT代码也分配给非点源文件中的源类别。

在SMOKE中，MACT代码被视为6个字符的字符串，并且没有与该数字关联的内部层次结构。

<a id=2.3.9></a>

### 2.3.9来源类型：主要和112部分区域来源

对于点和非点毒物清单，每个来源都可以标记为“ [主要]”或“ [第112节区域]”，以输入到SMOKE（以下段落说明了如何将“ [区域]”一词应用于某点）源库存）。 《清洁空气法》将主要排放源定义为那些每年排放或可能排放10吨或以上任何一种有毒空气污染物或每年排放25吨或以上任何有毒空气污染物的固定设施。第112节的区域源包括空气中毒物排放低于《清洁空气法》第112条所定义的主要源阈值的设施，因此每年排放的单一有毒空气污染物少于10吨，或每年少于25吨的总和多种有毒的空气污染物。对于非点源，原则上存在另一种源类型：“ [other]”源类型；这种源类型的一个例子是野火。但是，这些来源类型在非重点毒物清单中与第112节区域来源的标签没有不同，因此，到目前为止，“ [其他]”来源类型尚未包含在SMOKE中。

关于混淆使用“ [area]”术语来描述点源的说明：在点库存中将源指定为112节区域源与SMOKE的区域处理类别没有任何关系。 SMOKE仍会使用Lat-lon位置和堆栈参数将属于部分112区域源的点源作为点源进行处理。

实际上，所有“ [主要]”来源都应仅出现在点毒物清单中，但在某些情况下，“ [主要]”来源会出现在非点状清单中（特别是在1996年的清单年（2001年7月版）中）。该库存）。因此，以点和非点有毒物质输入格式提供了源类型名称。

当应用基于MACT的控制因子时，将使用主要和112部分区域名称。这些控制因子是根据来源的MACT代码分配的，并且可以仅应用于主要来源，仅适用于112部分区域来源，或者适用于两种类型的来源，无论其名称如何。

<a id=2.3.10></a>

### 2.3.10来源类型：非公路和公路移动来源

当应用基于MACT的控制因素时，将使用非公路和公路移动源类型名称。这些控制因子是根据源的MACT代码分配的。
...
profiles and the spatial "[profiles]", which are called spatial
...
rather assigns generic "[factors]" from profiles 15, 16, and 17
...
cross-reference entries are grouped according to the "[level]"
...
-   Inventory import (**Smkinven**)
    ...
    -   cross-references: `NHAPEXCLUDE`, `VMTMIX`,
        `PSTK`, `ARTOPNT`
...
-   Temporal allocation (**Temporal**)
    ...
    -   cross-references: `ATREF`, `MTREF`,
        `PTREF`
    -   profiles: `ATPRO`, `MTPRO`, `PTPRO`
    
-   Chemical speciation (**Spcmat**)
     
    -   cross-references: `GSREF`, `GSCNV`
    -   profiles: `GSPRO`
    
-   Spatial allocation (**Grdmat**)
     
    -   cross-references: `AGREF`, `MGREF`
    -   profiles: `AGPRO`, `MGPRO` (**\* Note**)
    
-   Growth and controls (**Cntlmat**)
     
    -   cross-references: `GCNTL`
    -   profiles: none (factors are included in the cross-reference
        files)
    
-   Mobile-source speed assignment (**Movesmrg**)
     
    -   cross-references: `MCXREF`, `MFMREF`
    -   profiles: `SPDPRO`
...
**Note**: The use of the Environment variable `AGPRO` (Area
spatial surrogate file)and `MGPRO` (Mobile spatial surrogate
file) have been discontinued. Two new Environment variables have been
introduced to SMOKE; `SRGPRO_PATH` (spatial surrogate profile
file location) and `SRGDESC` (description file with the specific
list of available surrogates located in `SRGPRO_PATH`) See
[Figure 6.4, "Grdmat input and output
files"](ch06s04s03.html#fig_programs_grdmat "Figure 6.4. Grdmat input and output files").
The surrogate files located in `SRGPRO_PATH` are refinements of
...
the `STACK_GROUPS` file, described in [Section 9.3.2,
"`STACK_GROUPS`"](ch09s03s02.html "9.3.2. STACK_GROUPS").) In
...
delimiters; in SMOKE, valid delimiters are [**spaces**],
[**commas**], or [**semicolons**]. If a particular field
...
and one with 20 variables. This resulting "[file set]" will be
...
emissions. In SMOKE, it is called the `EGTS3D_L` file. To create
...
`STACK_GROUPS` file. The second must contain the hourly,
...
Fortran binary file (called the `UAM_EGTS` file) by converting a
2-D `EGTS_L` file from an I/O API format. To obtain the
...
collectively as "[modeling parameters]". In this section, we
...
### 2.6.1. Map projections and model grids 
...
The model grid is set in SMOKE using the `IOAPI_GRIDNAME_1`
setting to select a grid and map projection from among those defined in
the `GRIDDESC` input file. The name of the grid set with the
`IOAPI_GRIDNAME_1` setting must match a grid name in the
`GRIDDESC` file to allow SMOKE to obtain the grid and map
projection parameters from the `GRIDDESC` file.
...
-   The `YEAR` setting in the SMOKE Assigns file is the
...
-   The episode and run settings (see [Section 2.6.3, "Modeling
    ...
    This base year must match the `YEAR` setting so that the
...
-   The input emissions files should ideally contain data for the same
    ...
    consistent with the `YEAR` environment variable in the
...
-   The setting `FYEAR` is set in the run script and is used by
    the script to automatically assign the name of the **Cntlmat** input
    file `GCNTL`, which contains the growth factors.
    `FYEAR` must be set to the future year even if a future-year
...
-   If you already have a future-year inventory and so do not need to
    ...
    case, the `SMK_BASEYR_OVERRIDE` setting must also be used to
...
files) be created by SMOKE. We use the term "[run period]" to
...
-   The episode start date (`EPI_STDATE`), episode start time
    (`EPI_STTIME`), episode duration in hours
    (`EPI_RUNLEN`), and the episode number of days
    (`EPI_NDAY`) all must be set to cover the modeling episode.
...
-   The start date of the first run period needs to be set using the
    `G_STDATE` and `ESDATE` settings. The
    `G_STDATE` is the year and Julian day setting used by the
    SMOKE programs; in our example above, `G_STDATE` would be
    set to 1996183, since July 1 is the 183rd day of 1996. The
    `ESDATE` is the Gregorian date used in naming the SMOKE
    intermediate and output files; for our example, `ESDATE`
    ...
    the episode, starting with the first `G_STDATE` value in the
    Assigns file. The `G_STDATE` and `ESDATE` settings
    are changed for each run period.

-   The run period start time (`G_STTIME`) and duration
    (`G_RUNLEN`) must also be set to indicate the start time and
    length of each run period. Both values are provided as a number of
    hours, using a HHMMSS (hours, minutes, seconds) format.

    The run period duration (`G_RUNLEN`) is usually not the same
    as the episode duration (`EPI_RUNLEN`). For example, if the
...
-   The `NDAYS`, `MSDATE`, and `MDAYS` settings
    are used for naming files. The `NDAYS` setting should be set
    to the number of days in each run period, and is used by default for
    naming time-based files. The `NDAYS` setting is also used
    along with the `EPI_NDAY` setting to loop through the run
    periods in the episode. The `MSDATE` and `MDAYS`
...
-   The AQMs supported by SMOKE always need one extra hour in each
    ...
    the `G_RUNLEN` setting should be 250000 for 25 hours.
...
-   All times are associated with a time zone, including the episode and
    ...
    Time (GMT); therefore, the `EPI_STDATE`,
    `EPI_STTIME`, `G_STDATE`, and `G_STTIME`
...
(`INVTABLE`), speciation profiles (`GSPRO`), speciation
cross-reference (`GSREF`), and the mobile processes file
(`MEPROC`) when creating on-road mobile emissions with MOVES
through SMOKE.
...
ASCII elevated-point-source file (`ELEVTS_L` or
`ELEVTS_S`) with the **Smkmerge** program. The way SMOKE obtains
...
structure included in the header of the `GRID_CRO_3D`
